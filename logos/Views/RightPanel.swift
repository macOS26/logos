...
struct LayersPanel: View {
    ...
    private func handleLayerDrop(draggedData: LayerDragData, targetIndex: Int) -> Bool {
        ...
        // PROTECT PASTEBOARD LAYER: Never allow Pasteboard layer (index 0) to be moved
        if sourceIndex == 0 {
            print("🚫 Cannot move Pasteboard layer - it must remain at the bottom")
            return false
        }
        
        // PROTECT CANVAS LAYER: Never allow Canvas layer (index 1) to be moved  
        if sourceIndex == 1 {
            print("🚫 Cannot move Canvas layer - it must remain above pasteboard")
            return false
        }
        
        // PROTECT PASTEBOARD LAYER: Never allow any layer to be moved to Pasteboard position (index 0)
        if targetIndex == 0 {
            print("🚫 Cannot move layers below Pasteboard layer")
            return false
        }
        
        // PROTECT CANVAS LAYER: Never allow any layer to be moved to Canvas position (index 1)
        if targetIndex == 1 {
            print("🚫 Cannot move layers to Canvas position")
            return false
        }
    ...
    private func handleObjectDrop(draggedData: ObjectDragData, targetLayerIndex: Int) -> Bool {
        ...
        // PROTECT CANVAS LAYER: Never allow objects to be moved to Canvas layer (index 1)
        if targetLayerIndex == 1 && targetLayerIndex < document.layers.count && document.layers[targetLayerIndex].name == "Canvas" {
