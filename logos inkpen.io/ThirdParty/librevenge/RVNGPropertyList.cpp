...
bool findDouble(const librevenge::RVNGString &str, double &res, librevenge::RVNGUnit &unit)
{
	...
	else if (first + 2 <= last && first[0] == 'p' && first[1] == 't')
	{
		unit = librevenge::RVNG_POINT; first += 2;
	}
	else if (first + 2 <= last && first[0] == 'i' && first[1] == 'n')
	{
		unit = librevenge::RVNG_INCH; first += 2;
	}
	else if (first + 1 <= last && first[0] == '*')
	{
		unit = librevenge::RVNG_TWIP; first += 1;
	}
	else if (first + 1 <= last && first[0] == '%')
	{
		unit = librevenge::RVNG_PERCENT; ratio = 100.0; first += 1;
	}
	else if (first + 2 <= last && first[0] == 'c' && first[1] == 'm')
	{
		unit = librevenge::RVNG_INCH; ratio = 2.54; first += 2;
	}
	else if (first + 2 <= last && first[0] == 'm' && first[1] == 'm')
	{
		unit = librevenge::RVNG_INCH; ratio = 25.4; first += 2;
	}
	else
	{
		unit = librevenge::RVNG_GENERIC;
...
bool findInt(const librevenge::RVNGString &str, int &res)
{
	...
	if (first == last) return false;
	char *endp = nullptr;
	long v = std::strtol(first, &endp, 10);
	if (endp == first) return false;
	first = endp;
	first = skipSpaces(first, last);
	if (first != last) return false;
...
bool findBool(const librevenge::RVNGString &str, bool &res)
{
	...
	const char *const last = first + str.size();
	first = skipSpaces(first, last);
	size_t remaining = (size_t)(last - first);
	auto ieq = [](const char *a, const char *b, size_t n) {
		for (size_t k = 0; k < n; k++)
			if (std::tolower((unsigned char)a[k]) != (unsigned char)b[k]) return false;
		return true;
	};
	if (remaining >= 4 && ieq(first, "true", 4)) { res = true; first += 4; }
	else if (remaining >= 5 && ieq(first, "false", 5)) { res = false; first += 5; }
	else return false;
	first = skipSpaces(first, last);
	return first == last;
}
