...
extension FileOperations {
    ...
    // MARK: - Extended SVG Import with Text Support
    
    @MainActor
    static func openSVGFileWithText(url: URL) async throws -> VectorDocument {
        Log.fileOperation("🔄 Opening SVG file with text support: \(url.lastPathComponent)", level: .info)
        ...
        // Import SVG using professional parser with extreme value handling
        let result = await VectorImportManager.shared.importSVGWithExtremeValueHandling(from: url)
        ...
        // Text objects are imported as shapes with isTextObject=true, so count them
        let textShapeCount = result.shapes.filter { $0.isTextObject }.count
        let regularShapeCount = result.shapes.filter { !$0.isTextObject }.count
        Log.fileOperation("📊 Imported \(regularShapeCount) shapes and \(textShapeCount) text objects from SVG", level: .info)
        ...
        // Add all shapes (including text shapes) to unified system
        for shape in result.shapes {
            // Center shape in document
            let docCenter = CGPoint(x: document.settings.width * 72.0 / 2.0, y: document.settings.height * 72.0 / 2.0)
            let shapeBounds = shape.bounds
            let shapeCenter = CGPoint(x: shapeBounds.midX, y: shapeBounds.midY)
            
            let offset = CGPoint(x: docCenter.x - shapeCenter.x, y: docCenter.y - shapeCenter.y)
            
            var centeredShape = shape
            centeredShape.transform = shape.transform.translatedBy(x: offset.x, y: offset.y)
