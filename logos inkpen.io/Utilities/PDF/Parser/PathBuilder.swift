...
func extractPDFVectorContent(_ page: CGPDFPage) throws -> PDFContent {
    ...
    if let pdfDoc = page.document,
       let info = pdfDoc.info {
        // Try to get Producer field
        var pdfString: CGPDFStringRef?
        if CGPDFDictionaryGetString(info, "Producer", &pdfString),
           let producerString = CGPDFStringCopyTextString(pdfString!) as String? {
            producer = producerString
            if producerString.hasPrefix("INKPEN_DATA:") {
                Log.info("📦 Found inkpen metadata in PDF Producer field", category: .fileOperations)
            }
        }
    }
