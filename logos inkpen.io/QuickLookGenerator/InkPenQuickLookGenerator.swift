...
//  InkPenQuickLookGenerator
//
//  Standalone QuickLook generator for .inkpen files
//
...
@main
struct InkPenQuickLookGenerator {
...
class Generator {
    ...
    func run() {
        ...
        guard args.count >= 3 else {
            print("Usage: InkPenQuickLookGenerator <input-file> <output-file>")
            exit(1)
        }
        ...
        do {
            try generatePreview(for: inputURL, to: outputURL)
    ...
    private func generatePreview(for inputURL: URL, to outputURL: URL) throws {
    ...
    private func createPreviewImage(from svgContent: String, size: CGSize) -> NSImage {
        ...
        // Create a simple preview showing document info
